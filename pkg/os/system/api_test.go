...
func TestWaitUntilServiceState_Success(t *testing.T) {
	...
	stateCheck := func() (bool, string, ServiceInterface, error) {
		if stateChanged {
			svc.State = serviceStateRunning
			return true, svc.State, svc, nil
		}
		return false, svc.State, svc, nil
	}
	...
	state, err := impl.WaitUntilServiceState(stateTransition, stateCheck, 10*time.Millisecond, 500*time.Millisecond)
	if err != nil {
...
func TestWaitUntilServiceState_Timeout(t *testing.T) {
	...
	impl := ServiceManagerImpl{}
	state, err := impl.WaitUntilServiceState(stateTransition, stateCheck, 10*time.Millisecond, 50*time.Millisecond)
	if !errors.Is(err, wmierrors.Timedout) {
...
func TestWaitUntilServiceState_TransitionFails(t *testing.T) {
	...
	impl := ServiceManagerImpl{}
	_, err := impl.WaitUntilServiceState(stateTransition, stateCheck, 10*time.Millisecond, 50*time.Millisecond)
